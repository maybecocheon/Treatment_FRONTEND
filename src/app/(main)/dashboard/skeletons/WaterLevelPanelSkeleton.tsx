...
export default function WaterLevelPanelSkeleton() {
  const skeletonRow = Array(6).fill(0);

  return (
    <div className="glass rounded-4xl p-6 h-full flex flex-col min-h-0 overflow-hidden">
      {/* 타이틀 영역 스켈레톤 */}
      ...
      <div className="flex-1 relative flex flex-col justify-start items-center pt-2 gap-4">
        ...
        <div className="w-full flex flex-col gap-4">
          ...
          <div className="flex justify-center gap-3 w-full">
            {skeletonRow.map((_, i) => (
              <div key={`row1-${i}`} className="flex-1 max-w-30">
                <div className="w-full h-20 rounded-2xl bg-slate-100/50 border-2 border-slate-100 animate-pulse" />
              </div>
            ))}
          </div>
          ...
          <div className="flex justify-center gap-3 w-full">
            {skeletonRow.map((_, i) => (
              <div key={`row2-${i}`} className="flex-1 max-w-30">
                <div className="w-full h-20 rounded-2xl bg-slate-100/50 border-2 border-slate-100 animate-pulse" />
              </div>
            ))}
          </div>
