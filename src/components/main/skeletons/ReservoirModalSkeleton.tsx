        ...
        {[1, 2].map((i) => (
          <div key={i} className="glass p-6 rounded-3xl h-32 flex flex-col justify-between">
            ...
            <div className="flex items-end justify-between">
              ...
              <div className="h-6 w-12 bg-card-border rounded-full animate-pulse" />
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="glass rounded-3xl p-6 h-130 flex flex-col">
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex gap-2">
            {[1, 2, 3].map((j) => (
              <div key={j} className="h-7 w-12 bg-card-border rounded-2xl animate-pulse" />
            ))}
          </div>
        </div>
        {/* 차트 그래픽 */}
        <div className="flex-1 w-full bg-card-border/30 rounded-2xl relative overflow-hidden">
          <div className="absolute inset-0 flex items-center justify-around px-4">
            {[...Array(6)].map((_, idx) => (
              <div
                key={idx}
                className="w-12 bg-muted/10 rounded-t-lg animate-pulse"
