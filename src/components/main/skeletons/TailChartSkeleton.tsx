  ...
  return (
    <div className="w-full h-full flex flex-col gap-4 animate-pulse">
      ...
      <div className="flex-1 w-full relative">
        <div className="absolute inset-0 bg-slate-100/50 rounded-2xl border border-slate-100 flex items-end p-4">
          {/* 차트 격자 느낌을 주기 위한 가로선들 */}
          <div className="w-full h-full flex flex-col justify-between pt-5 pb-8">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="w-full border-t border-slate-200/50" />
            ))}
          </div>
          ...
          <div className="absolute left-1 top-10 bottom-10 flex flex-col justify-between">
             {[1, 2, 3].map((i) => <div key={i} className="h-2 w-6 bg-slate-200 rounded" />)}
          </div>
