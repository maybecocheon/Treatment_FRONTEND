    ...
    <header className="h-20 backdrop-blur-xl sticky top-0 z-50 transition-all">
      <div className="glass max-w-8xl mx-auto h-full flex items-center justify-between px-4 md:px-8">
        ...
        <div className="flex items-center gap-3 md:gap-6">
          ...
          <button
            onClick={addOneHour}
            className="hidden sm:flex items-center gap-1.5 px-3 py-2 bg-white border border-slate-200 text-slate-600 rounded-2xl text-xs font-bold hover:border-sky-300 hover:text-sky-600 hover:bg-sky-50 transition-all active:scale-95 shadow-sm group"
          >
            <ClockAlert className="w-3.5 h-3.5 text-amber-500 group-hover:animate-pulse" />
            <span>+1시간</span>
          </button>
          ...
          {isProfileLoading ? (
              ...
              {showProfile && (
                <div className="absolute right-0 mt-3 w-60 bg-white border border-slate-100 rounded-3xl shadow-2xl shadow-slate-200/60 p-2 z-50 animate-in fade-in slide-in-from-top-2">
                  <div className="p-1 space-y-1">
                    <button 
                      className="w-full flex items-center space-x-3 px-4 py-3 text-sm font-bold text-slate-600 hover:bg-sky-50 hover:text-sky-600 rounded-xl transition-colors"
            ...
            <div className="text-xs text-slate-400">로그인이 필요합니다.</div>
          )}
